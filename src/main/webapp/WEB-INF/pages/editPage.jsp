...
<form class="style" action="${empty film.title ? addUrl : editUrl}" name="film" method="POST">
    <c:choose>
        <c:when test="${!empty film.title}">
            <p class="heading">Редактировать фильм</p>
            <input type="hidden" name="id" value="${film.id}">
        </c:when>
        <c:otherwise>
            <p class="heading">Добавить фильм</p>
        </c:otherwise>
    </c:choose>
    ...
    <p><input type="text" name="genre" placeholder="Жанр" value="${film.genre}" maxlength="20" required>
    <p class="checkbox">
        <label for="watched">Смотрел
